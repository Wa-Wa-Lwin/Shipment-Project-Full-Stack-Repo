...
class SAPSalesInvoiceListController
{
     ...
     * GET /api/logistics/sap_enterpryze/get_all_sales_invoices?page=1&per_page=20
     */
    public function getAllSalesInvoices(Request $request, int $retryCount = 0)
    {
        try {
            ...
            $countResponse = Http::withOptions(['verify' => false])
                ...
                ->get("{$this->sapUrl}/Invoices/\$count");

            $total = $countResponse->successful() ? (int) $countResponse->body() : 0;

            // Fetch page of invoices (only fields needed for the list view)
            $response = Http::withOptions(['verify' => false])
                ->withCookies($session['cookies'], '192.168.68.16')
                ->get("{$this->sapUrl}/Invoices", [
                    '$select' => 'DocEntry,DocNum,CardCode,CardName,DocDate,DocDueDate,DocTotal,DocCurrency,DocumentStatus,NumAtCard,CreationDate',
            ...
            if ($this->isSessionTimeout($response) && $retryCount < 1) {
                Log::warning('SAP session timeout on getAllSalesInvoices, retrying after re-login');
                $this->clearSessionCache();

                return $this->getAllSalesInvoices($request, $retryCount + 1);
            }

            if (! $response->successful()) {
                Log::error('SAP Invoices list fetch failed', [
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to fetch sales invoices from SAP',
            ...
            Log::info('SAP Invoices list fetched', [
                'page' => $page,
                'per_page' => $perPage,
            ...
            Log::error('SAP getAllSalesInvoices exception', [
